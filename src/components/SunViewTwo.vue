...
import { basicThree2 } from '../core/basicThree2'
import { useSun } from '../hooks/sun'
import { solarTerms } from '../help/constant'
import { useTag } from '@/hooks/useTag'
import { tweenCamera } from '@/help/animate'
import { hideDom } from '@/help/utils'

let sun_imgUrl = require('../assets/sun.png')
  ...
  document.addEventListener('mouseup', (event) => {
    if (!isDragging) {
      ...
      if (intersects.length > 0) {
        ...
        while (clickedObject && clickedObject.type !== 'Group') {
        ...
        const targetGroups = scene.children.filter(child => child.isGroup)
        targetGroups.forEach(group => {
          if (group.uuid !== clickedObject.uuid) {
...
const backSandbox = () => {
  ...
  // 恢复模型透明度
  const targetGroups = threeObj.scene.children.filter(child => child.isGroup)
  targetGroups.forEach(group => {
    group.traverse((child) => {
      if (child.isMesh) {
...
const changeModelColor2 = (isDefault) => {
  const { scene } = threeObj
...
const changeModelColor = (isDefault) => {
  ...
  targetGroups.forEach(group => {
    group.traverse((child) => {
      if (child.isMesh) {
        if (isDefault) {
          child.material.color.setHex(0xfffff0)
        } else {
          child.material.color.copy(child.material.originColor)
        }
      }
    });
  })
          ...
          <van-slider v-model="progress" style="width: 70%" :step="5" :min="0" active-color="#eaedee"
            ...
            <template #button>
              <div class="custom-sun-slider">
                ...
                <img :src=sun_imgUrl ondragstart="return false" alt="" />
              </div>
            </template>
          </van-slider>
